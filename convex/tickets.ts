...
export const scanTicket = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (ticket.checkinCounter && ticket.checkinCounter > 0 && (now - lastScanTime) < 3000) {
      // Return current state without incrementing if scanned within 3 seconds
      return {
        status: "warning",
        message: ticket.checkinCounter === 1 ? "Welcome! You are already checked in" : `Ticket scanned multiple times (${ticket.checkinCounter} attempts)`,
        ticket: {
    ...
    if (currentCounter === 0) {
      ...
      return {
        status: "warning",
        message: currentCounter === 1 ? "Welcome! You are already checked in" : `Ticket scanned multiple times (${newCounter} attempts)`,
        ticket: {
          teamName: ticket.teamName,
          leaderName: ticket.leaderName,
          checkedInAt: ticket.checkedInAt,
